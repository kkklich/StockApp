...
{
    Task<string> RenderViewToStringAsync<TModel>(string viewDirectory, string viewName, TModel model);
}
...
public class RazorTemplateGenerator : IRazorTemplateGenerator
{
    ...
    public async Task<string> RenderViewToStringAsync<TModel>(string viewDirectory, string viewName, TModel model)
    {
        var actionContext = GetActionContext();
        var view = FindView(actionContext, viewName, viewDirectory);
    ...
    private IView FindView(ActionContext actionContext, string viewName, string viewDirectory)
    {
        if (!_hostingEnvironment.IsProduction()) viewName = $"{viewDirectory}{Path.DirectorySeparatorChar}{viewName}";
